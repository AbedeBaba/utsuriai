...
serve(async (req) => {
  ...
  try {
    ...
    if (!apiKey) {
      const keyName = usePro ? 'NANO_BANANA_PRO_API_KEY' : 'LOVABLE_API_KEY';
      console.error(`${keyName} is not configured`);
      return new Response(
        JSON.stringify({ error: `${usePro ? 'Pro' : 'AI'} API key not configured` }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    
    console.log(`Using ${usePro ? 'Nano Banana Pro' : 'Lovable'} API with model: ${model}`);
    ...
    if (!aiResponse.ok) {
      ...
      if (aiResponse.status === 401) {
        return new Response(
          JSON.stringify({ error: 'API key is invalid or unauthorized.' }),
    ...
    console.log('AI response received with', usePro ? 'Pro' : 'Standard', 'quality');
